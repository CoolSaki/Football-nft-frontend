...
const PassPhrase: React.FC<Props> = ({ onClose, transactionData }) => {
  ...
  const countDownDate = new Date('Jan 5, 2024 00:15:00').getTime()
  ...
  const { transactionStatus, send, initialize } = useContext(ConnectContext)
  ...
  useEffect(() => {
    if (isRestrictedSecret === 'true') {
      dispatch(sendMaticRestrictSecret())
    }
    return () => {
  ...
  useEffect(() => {
    console.log({ isSendMaticSuccess })
    if (isSendMaticSuccess !== '' && isSendMaticError === '') {
      send(
        isSendMaticSuccess,
    ...
    if (sendMaticTxnId) {
      ...
      interval = setInterval(function () {
        if (new Date().getTime() - startTime > 60000) {
          setTransactionFailure(t('transaction failed'))
          clearInterval(interval)
          return
        }
        //do whatever here..
